...
let prevNotice = require('../model/cron')
module.exports = async function (date, today, socket, axios) {
	let start = Number(date.getHours()) + 1;
	
	let groups = await prevNotice.getGroupsByDay(today, start);
	
	for(let i = 0; i<groups.length; i++) {
		let group_info = await prevNotice.getGroupInfoById(groups[i].group_id)
		let students_info = await prevNotice.getStudentsInfoByGroup(groups[i].group_id);
		...
		if(!delivered) {
			let ins;
			if(group_type) 
				await prevNotice.insertNotice(teacher_id, 'В '+ start+':00 у вас начнется занятие со студентом <a href="/student?id='+student_id+'">'+lastname +' '+ firstname +'</a>!', 0)
			else
				await prevNotice.insertNotice(teacher_id, 'В '+ start+':00 у вас начнется занятие с группой <a href="/group?id='+group_id+'">'+group_name+'</a>!', 0);
		}
		...
		for (let j=0;j<students_info.length;j++) {
			...
			socket.clients.forEach( (ws) => {
					if (ws.userid === students_info[j]["id"]) {
						ws.send(JSON.stringify({group_id, notice: 0, text: 'Ваше занятие начнется через один час!'}));
						delivered = true;
					}
				});
			...
			if(!delivered) {
				let ins;
				if(group_type) 
					await prevNotice.insertNotice(students_info[j]["id"], 'Ваше занятие начнется через один час!' , 1)
				else
					await prevNotice.insertNotice(students_info[j]["id"], 'Ваше занятие начнется через один час!' , 1);
			}
		}
