...
module.exports = async function (date, today, socket, axios) {
	...
	for(let i = 0; i<groups.length; i++) {
		...
		if(group_type) 
			body.msg = 'Закончилось занятие со студентом <a href="/student?id='+student_id+'">'+lastname +' '+ firstname+'</a>! <a href="/chat"> перейти к чату</a>';
		else body.msg = 'Закончилось занятие с группой <a href="/group?id='+group_id+'">'+group_name+' </a>! <a href="/chat"> перейти к чату</a>';
		...
		if(!delivered) {
			let ins;
			if(group_type) 
				await prevNotice.insertNotice(teacher_id, 'У вас закончилось занятие со студентом <a href="/student?id='+student_id+'">'+lastname +' '+ firstname +'</a>!', 0)
			else
				await prevNotice.insertNotice(teacher_id, 'У вас закончилось занятие с группой <a href="/group?id='+group_id+'">'+group_name+'</a>!', 0);
		}
		...
		for (let j=0;j<students_info.length;j++) {
			...
			socket.clients.forEach( (ws) => {
					if (ws.userid === students_info[j]["id"]) {
						ws.send(JSON.stringify({group_id, notice: 0, text: 'Ваше занятие закончилось!'}));
						delivered = true;
					}
				});
			...
			if(!delivered) {
				let ins;
				if(group_type) 
					await prevNotice.insertNotice(students_info[j]["id"], 'Ваше занятие закончилось!' , 1)
				else
					await prevNotice.insertNotice(students_info[j]["id"], 'Ваше занятие закончилось!' , 1);
			}
		}

		await lessonOpen.updateStream(group_id, 0);
